...
export function getTotalActiveMembers(familyData: any) {
  ...
  export async function getMyFamily(
    setFamily: (value: React.SetStateAction<Family | null>) => void,
    setTotalMembers: (value: React.SetStateAction<number>) => void,
    setTotalActiveMembers?: React.Dispatch<React.SetStateAction<number>>,
  ) {
    try {
      const res = await authorizedAPI.get('/families/mine');
      const familyData = res.data; // Extract the response data
      ...
      const totalMembersCount = familyData.members.length;

      // Count active members based on attendance records
      const totalActiveMembers =
        familyData.attendances?.reduce((count: number, record: any) => {
          return count + (record.abaje || 0);
        }, 0) || 0;
...
export async function handleAddFamily(
  ...
  if (familyForm.name && familyForm.father && familyForm.mother) {
    try {
      const res = await authorizedAPI.post('/families', {
...
export async function handleSaveChanges(
  ...
  if (familyForm && selectedFamily) {
    try {
      const res = await authorizedAPI.put(`/families/${selectedFamily.id}`, {
        familyName: familyForm.name,
        father: familyForm.father,
        mother: familyForm.mother,
      });
...
export async function handleDeleteFamily(
  ...
  if (selectedFamily) {
    try {
      const res = await authorizedAPI.delete(`/families/${selectedFamily.id}`);
      if (res.status === 200) {
...
export async function fetchAttendances(
  ...
  setAttendances: React.Dispatch<
    React.SetStateAction<Record<string, AttendanceRecord[]> | null>
    >,
  ) {
  try {
    setLoading(true);
