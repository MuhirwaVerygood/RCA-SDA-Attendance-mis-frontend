  ...
  const AttendanceReport = ({
   ...
   const famPercentage =
     isParent && attendanceData.length > 0
       ? calculateAttendancePercentage(attendanceData[0], features)
       : '0%';
    ...
    if (attendanceData.length == 0) return <p>No attendance available for the selected date</p>
          ...
          <tbody>
            ...
            <tr className="bg-gray-200 font-bold">
              ...
              {families.map((family, index) => {
                // Determine the percentage to display based on the user role
                const percentageToDisplay = user?.isAdmin
                  ? familyPercentages[index] // Use the percentage from familyPercentages for admin
                  : isParent && family === attendanceData[0]?.family.familyName // Only display famPercentage for the parent's family
                  ? famPercentage
                  : 'N/A'; // Display "N/A" for other families if the user is a parent

                const numericPercentage = parseFloat(percentageToDisplay);

                return (
                  <td
                    key={index}
                    className={`border text-center border-gray-300 px-4 py-2 ${
                      numericPercentage < 50 ? 'text-red-500' : 'text-black'
                    }`}
                  >
                    {percentageToDisplay}
                  </td>
                ...
                className={`border text-center border-gray-300 px-4 py-2 ${
                  user?.isAdmin
                    ? parseFloat(overallPercentage) < 50
                      ? 'text-red-500'
                      : 'text-black'
                    : parseFloat(famPercentage) < 50
                    ? 'text-red-500'
                    : 'text-black'
                }`}
              >
                {user?.isAdmin ? overallPercentage : famPercentage}
              </td>
            </tr>
          </tbody>
        </table>
